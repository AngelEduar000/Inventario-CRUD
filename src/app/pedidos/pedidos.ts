...
      p.id_pedido.toLowerCase().includes(term) || // id_pedido es string ahora
...
    return {
      id_pedido: '', // CORREGIDO: Inicializa el campo requerido para PedidoCreate
      cedula: '',
      id_producto: '',
...
        // --- Creando (POST) ---
        
        // CORRECCIÓN: Generar ID de 10 caracteres o menos para cumplir con VARCHAR(10).
        // P- (2 chars) + últimos 8 dígitos del timestamp = 10 caracteres.
        const idGenerado = this.currentPedido.id_pedido.trim() || `P-${Date.now().toString().slice(-8)}`; 

        const payload: PedidoCreate = {
            id_pedido: idGenerado, // AÑADIDO: El ID es obligatorio
            cedula: this.currentPedido.cedula,
            id_producto: this.currentPedido.id_producto,
            fecha_entrega: this.currentPedido.fecha_entrega,
            cantidad: this.currentPedido.cantidad,
...
    // CORREGIDO: Se elimina window.confirm() por violación de restricción.
    // En una aplicación real, se usaría un modal de confirmación aquí.
    if (!id) {
        this.showMessage('ID de pedido no válido para eliminar.', 'error');
        return;
    }
